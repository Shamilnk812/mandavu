...
export default function SignUp() {
  ...
  <div class="py-12 px-12 bg-white rounded-2xl shadow-xl z-20">
    ...
    <form onSubmit={formik.handleSubmit} class="space-y-4">
      <input type="text" name='first_name' value={formik.values.first_name} onChange={formik.handleChange} placeholder="First Name" class="block text-sm py-3 px-4 rounded-lg w-full bg-white border border-gray-300 outline-teal-500" />
      {formik.errors.first_name && formik.touched.first_name ? (
              <div className="text-red-500 text-sm">{formik.errors.first_name}</div>
            ) : null}

      <input type="text" name='last_name' value={formik.values.last_name} onChange={formik.handleChange} placeholder="Last Name" class="block text-sm py-3 px-4 rounded-lg w-full bg-white border border-gray-300 outline-teal-500" />
      ...
      <input type="text" name='convention_center_name' value={formik.values.convention_center_name} onChange={formik.handleChange} placeholder="Convention Center Name" class="block text-sm py-3 px-4 rounded-lg w-full bg-white border border-gray-300 outline-teal-500" />
      ...
      <input type="password" name='password2'  value={formik.values.password2} onChange={formik.handleChange}   placeholder="Confirm Password" class="block text-sm py-3 px-4 rounded-lg w-full bg-white border border-gray-300 outline-teal-500" />
      {formik.errors.password2 && formik.touched.password2 ? (
              <div className="text-red-500 text-sm">{formik.errors.password2}</div>
            ) : null}

    <div class="text-center mt-6">
      <button type='submit' class="w-full py-2 text-xl text-white bg-teal-500 rounded-lg hover:bg-teal-600 transition-all">Create Account</button>
