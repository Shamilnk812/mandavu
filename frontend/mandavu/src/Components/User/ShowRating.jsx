...
<div className="mt-16 rounded-lg bg-customColor8 lg:h-[400px] h-auto">
  <div className="text-center rounded-tl-lg rounded-tr-lg py-3 bg-gradient-to-r from-teal-500 to-gray-800">
    <p className="text-xl font-semibold text-white">Review & Ratings</p>
  </div>

  <div className="pl-4 py-5 md:flex md:justify-between md:pl-10">
    {reviews.length > 0 ? (
      <div className="w-full md:w-2/5 bg-white p-4 rounded shadow-lg mb-6 md:mb-0">
        <div className="flex justify-center mb-6">
          <h2 className="font-semibold text-gray-800 text-xl">Overall Rating</h2>
        </div>
        <div className="flex items-center mb-2">
          ...
          {Array.from({ length: 5 }).map((_, index) => (
            <svg
              key={index}
              className={`w-4 h-4 ${
                index < Math.round(ratingData.overall_rating)
                  ? "text-yellow-300"
                  : "text-gray-300"
              } me-1`}
          ...
          <div key={item.rating} className="flex items-center mt-4">
            ...
            <div className="w-2/4 h-5 mx-4 bg-gray-200 rounded dark:bg-gray-700">
              <div
                ...
                style={{
                  width: `${
                    (item.count / ratingData.total_ratings) * 100
                  }%`,
                }}
              ></div>
            </div>
    ...
    <div className="py-5 md:px-10 px-4 w-full">
      {reviews.length > 0 ? (
        reviews.map((review) => (
          <div
            key={review.id}
